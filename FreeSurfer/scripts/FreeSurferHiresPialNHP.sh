...
#	MAXGRAY="-max_gray_at_csf_border 40"
  ...
  MAXTHICKNESS="-max 5"
elif [ "$SPECIES" = "Chimpanzee" ]; then
  # NOTE: 根据人，猕猴，狨猴推测的黑猩猩参数。by RJX
  VARIABLESIGMA="7"              # 人类4，黑猩猩7，猕猴10，狨猴8/20
  MAXTHICKNESS="-max 7"          # 人类5，黑猩猩7，猕猴10，狨猴20
  PSIGMA="-psigma 5"             # 人类？，黑猩猩5，猕猴5，狨猴10
  NSIGMA_ABOVE="-nsigma_above 3" # 人类？，黑猩猩3，猕猴3，狨猴7
else
...
if [ "$T2wFlag" != "NONE" ]; then
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.thickness`，无需从`lh.thickness.preT2`进行重命名。by RJX
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.area.pial`，无需从`lh.area.pial.preT2`进行重命名。by RJX
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.curv`，无需从`lh.curv.preT2`进行重命名。by RJX

...
if [ "$T2wFlag" != "NONE" ]; then
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.thickness`，无需从`lh.thickness.preT2`进行重命名。
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.area.pial`，无需从`lh.area.pial.preT2`进行重命名。
  ...
  # NOTE: 上面的`mri_surf2surf`命令中，输出名即为`lh.curv`，无需从`lh.curv.preT2`进行重命名。

...
# for hemisphere
# NOTE: 可能是版本问题，`--tval-xyz`后面必须跟一个volume文件，因此修改。by RJX
...
cp "$RescaleVolumeTransform".mat \
  "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/fs2real.mat
convert_xfm -omat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/real2fs.mat \
  -inverse "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/fs2real.mat
if [[ $NoT2wData -ne 1 ]]; then
  convert_xfm -omat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/temp.mat \
    -concat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/T2wtoT1w.mat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/real2fs.mat
  convert_xfm -omat "$SubjectDIR"/"$SubjectID"/mri/transforms/T2wtoT1w.mat \
    -concat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/fs2real.mat "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/temp.mat
  rm "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/temp.mat
else
  echo " "
  echo "没有T2w数据，以下步骤将跳过：在FS/<subj>/mri/transforms下生成T2wtoT1w.mat！"
fi
...
# NOTE: 需要将$FREESURFER_HOME/matlab添加到MATLAB路径中
hemi="lh"
matlab -nojvm -nosplash <<M_PROG
addpath $HCPPIPEDIR/global/matlab
corticalthickness('${surf}','${hemi}');

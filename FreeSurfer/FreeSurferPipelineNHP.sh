...
function runFSbrainmaskandseg() {
  ...
  # NOTE: “-gca-dir”默认找的是RB_all_2016-05-10.vc700.gca文件，但是没有。所以添加了“-gca”和“-gca-skull”，然后手动指定文件。
  recon-all \
    -subjid $SubjectID \
    -sd $SubjectDIR \
    -gcareg \
...
function runFSaseg() {
  ...
  DIR=$(pwd)
  cd "$SubjectDIR"/"$SubjectID"/mri
  # NOTE: 这个命令使用 FreeSurfer 中的 mri_cc 工具进行 胼胝体（Corpus Callosum, CC）分割和修正，并将其添加回 自动分割的 aseg 文件 中
...
function runNormalize2() {
  ...
  # NOTE: `-segmentation`功能：执行组织分割（Tissue Segmentation），将 MRI 体素分类为灰质（GM）、白质（WM）、脑脊液（CSF）等组织类型
  ...
  # NOTE: 将 屏状核（Claustrum） 的特定标签合并到白质（WM）图像中
  ...
  if [[ $ControlPoints = NONE ]]; then
    # NOTE: 目的是 通过掩膜运算修正白质（WM）图像的区域标记，可能用于处理屏状核（Claustrum）或相邻结构的分类问题
  ...
  if [[ $SPECIES =~ Marmoset || $SPECIES =~ Macaque ]]; then
    # NOTE: `$GCAdir``文件夹下没有`wmskeleton.nii.gz`文件，需要从`${HCPPIPEDIR_Templates}/MacaqueRIKEN21`拷贝一个。
...
function runFSwhite() {
  ...
  if [[ ! $SPECIES =~ Human ]]; then
    ...
    # NOTE: 没有`?h.white`文件，只有`?h.white.preaparc`文件（原因未知，可能是版本问题？），因此增加以下代码

...
${CARET7DIR}/wb_command -cifti-create-dense-timeseries \
	...
	-volume "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii.gz \
	"$ROIFolder"/Atlas_ROIs."$GrayordinatesResolution".nii.gz \
	-left-metric "$NameOffMRI"_s"$SmoothingFWHM".atlasroi.L."$LowResMesh"k_fs_LR.func.gii \
	-roi-left "$DownSampleFolder"/"$Subject".L.atlasroi."$LowResMesh"k_fs_LR.shape.gii \
	-right-metric "$NameOffMRI"_s"$SmoothingFWHM".atlasroi.R."$LowResMesh"k_fs_LR.func.gii \

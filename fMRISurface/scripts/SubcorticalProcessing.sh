...
# 根据ROIs.2.nii.gz，生成对应体素的.dtseries.nii文件，大小是“时间点 x 体素数量”
# generate subject-roi space fMRI cifti for subcortical
if [[ $(echo "$BrainOrdinatesResolution == $FinalfMRIResolution" | bc -l | cut -f1 -d.) == "1" ]]; then
  ...
  ${CARET7DIR}/wb_command -cifti-create-dense-timeseries \
    ${ResultsFolder}/${NameOffMRI}_temp_subject.dtseries.nii \
    -volume "$VolumefMRI".nii.gz \
...
${CARET7DIR}/wb_command -cifti-create-label \
  ${ResultsFolder}/${NameOffMRI}_temp_template.dlabel.nii \
...
if [[ $(echo "${Sigma} > 0" | bc -l | cut -f1 -d.) == "1" ]]; then
  ...
  ${CARET7DIR}/wb_command -cifti-smoothing \
    ...
    0 ${Sigma} \
    COLUMN \
    ${ResultsFolder}/${NameOffMRI}_temp_subject_smooth.dtseries.nii \
    -fix-zeros-volume
  ...
  ${CARET7DIR}/wb_command -cifti-resample \
    ...
    ADAP_BARY_AREA \
    CUBIC \
    ${ResultsFolder}/${NameOffMRI}_temp_atlas.dtseries.nii \
    -volume-predilate 10
...
${CARET7DIR}/wb_command -cifti-separate \
  ${ResultsFolder}/${NameOffMRI}_temp_atlas.dtseries.nii \
  COLUMN \
  -volume-all "$VolumefMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii.gz

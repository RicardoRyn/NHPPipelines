...
# HACK: 我们的`T1w_brain.nii.gz`文件在剥头皮质量特别好的情况下，可以直接使用brain文件来制作mask。by RJX
...
fslmaths "$T1wFolder"/T1w_acpc_dc_restore_brain.nii.gz -bin "$T1wFolder"/"$T1wImageBrainMask".nii.gz
...
for Hemisphere in L R; do
  ...
  if [ ${RegName} = "MSMSulc" ]; then
    ...
    # NOTE: 应该是版本原因，使用MSMSulcStrainFinalconf_by_rjx，否则会报错。by RJX
    ${MSMBINDIR}/msm \

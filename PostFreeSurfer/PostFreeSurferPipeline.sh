...
Speices=`opts_GetOpt1 "--species" $@`
NoT2wData=`opts_GetOpt1 "--not2wdata" $@`
...
"$PipelineScripts"/FreeSurfer2CaretConvertAndRegisterNonlinear.sh \
  "$StudyFolder" \
  "$Subject" \
  "$T1wFolder" \
  "$AtlasSpaceFolder" \
...
"$PipelineScripts"/CreateRibbon.sh \
  "$StudyFolder" \
  "$Subject" \
  "$T1wFolder" \
  "$AtlasSpaceFolder" \
...
if [[ $NoT2wData -ne 1 ]]; then
  "$PipelineScripts"/CreateMyelinMapsNHP.sh \
    "$StudyFolder" \
    "$Subject" \
    "$AtlasSpaceFolder" \
    "$NativeFolder" \
    ...
    "$HighResMesh" \
    "$LowResMeshes" \
    "$T1wFolder"/"$OrginalT1wImage" \
    "$T2wFolder"/"$OrginalT2wImage" \
    "$T1wFolder"/"$T1wImageBrainMask" \
    "$T1wFolder"/xfms/"$InitialT1wTransform" \
    "$T1wFolder"/xfms/"$dcT1wTransform" \
    "$T2wFolder"/xfms/"$InitialT2wTransform" \
    "$T1wFolder"/xfms/"$dcT2wTransform" \
    "$T1wFolder"/"$FinalT2wTransform" \
    "$AtlasTransform" \
    "$T1wFolder"/"$BiasField" \
    "$T1wFolder"/"$OutputT1wImage" \
    "$T1wFolder"/"$OutputT1wImageRestore" \
    "$T1wFolder"/"$OutputT1wImageRestoreBrain" \
    "$AtlasSpaceFolder"/"$OutputMNIT1wImage" \
    "$AtlasSpaceFolder"/"$OutputMNIT1wImageRestore" \
    "$AtlasSpaceFolder"/"$OutputMNIT1wImageRestoreBrain" \
    "$T1wFolder"/"$OutputT2wImage" \
    "$T1wFolder"/"$OutputT2wImageRestore" \
    "$T1wFolder"/"$OutputT2wImageRestoreBrain" \
    "$AtlasSpaceFolder"/"$OutputMNIT2wImage" \
    "$AtlasSpaceFolder"/"$OutputMNIT2wImageRestore" \
    "$AtlasSpaceFolder"/"$OutputMNIT2wImageRestoreBrain" \
    "$T1wFolder"/xfms/"$OutputOrigT1wToT1w" \
    "$T1wFolder"/xfms/"$OutputOrigT1wToStandard" \
    "$T1wFolder"/xfms/"$OutputOrigT2wToT1w" \
    "$T1wFolder"/xfms/"$OutputOrigT2wToStandard" \
    "$AtlasSpaceFolder"/"$BiasFieldOutput" \
    "$AtlasSpaceFolder"/"$T1wImageBrainMask" \
    "$AtlasSpaceFolder"/xfms/"$Jacobian" \
    "$ReferenceMyelinMaps" \
    "$CorrectionSigma" \
    "$RegName" \
    "$Speices"
  ...
  echo " "
  echo "没有T2w数据，以下步骤将跳过：生成髓鞘化map图！"
fi

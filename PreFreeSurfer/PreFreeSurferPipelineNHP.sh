...
NoT2wData=$(getopt1 "--not2wdata" $@)             # "$10" #是否有T2w的数据，如果没有则填1。by RJX
...
for TXw in ${Modalities}; do
  ...
  if [ ! $GradientDistortionCoeffs = "NONE" ]; then
    ...
    i=1
    for Image in $TXwInputImages; do
      ${RUN} ${FSLDIR}/bin/fslreorient2std $Image ${TXwFolder}/${TXwImage}${i}_gdc                           # 源代码导致路径重复了2遍而找不到文件。by RJX
      ${RUN} ${FSLDIR}/bin/fslreorient2std $(remove_ext $Image)_brain ${TXwFolder}/${TXwImage}${i}_gdc_brain # 源代码导致路径重复了2遍而找不到文件。by RJX
      OutputTXwImageSTRING="${OutputTXwImageSTRING}${TXwFolder}/${TXwImage}${i}_gdc "
      i=$(($i + 1))
    done
  fi
...
if [[ NoT2wData -ne 1 ]]; then
  ...
  echo " "
  echo "没有T2w数据，以下步骤将跳过：T2w图像配准到T1w图像！"
fi

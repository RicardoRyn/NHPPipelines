...
topup_config_file=${configdir}/b02b0_macaque.cnf

# NOTE: 为了更好地进行eddy_quad，增加“--fout”选项，参照人上的HCPpipelines
${FSLDIR}/bin/topup \
  --imain=${workingdir}/Pos_Neg_b0 \
  --datain=${workingdir}/acqparams.txt \
  --config=${topup_config_file} \
  --out=${workingdir}/topup_Pos_Neg_b0 \
  -v \
  --fout=${workingdir}/topup_Pos_Neg_b0_field.nii.gz
...
${FSLDIR}/bin/applytopup \
  --imain=${workingdir}/Pos_b01,${workingdir}/Neg_b01 \
  --topup=${workingdir}/topup_Pos_Neg_b0 \
  --datain=${workingdir}/acqparams.txt \
  --inindex=1,${dimt} \
  --out=${workingdir}/hifib0

...
if [[ -e "$BiasField" ]]; then
  ...
  echo "没有$BiasField文件（可能是因为没有T2w数据）。跳过此步骤"
  cp "$WorkingDirectory"/"$regimg"2T1w_initII.nii.gz \
    "$WorkingDirectory"/"$regimg"2T1w_restore_initII.nii.gz
fi
...
if [[ -e "$BiasField" ]]; then
  ...
  echo "没有$BiasField文件（可能是因为没有T2w数据）。跳过此步骤"
  cp "$WorkingDirectory"/"$regimg"2T1w.nii.gz \
    "$WorkingDirectory"/"$regimg"2T1w_restore.nii.gz
fi

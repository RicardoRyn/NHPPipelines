...
if [ ${CombineDataFlag} -eq 2 ]; then
	...
	${FSLDIR}/bin/eddy_combine \
		${eddydir}/eddy_unwarped_Pos \
		${eddydir}/Pos.bval \
		${eddydir}/Pos.bvec \
		${eddydir}/Pos_SeriesVolNum.txt \
		${eddydir}/eddy_unwarped_Neg \
		${eddydir}/Neg.bval \
		${eddydir}/Neg.bvec \
	...
	# RRRRRRR 增加“${CombineDataFlag}” by RJX on 2024/9/8 RRRRRRR
	# 将“NHPPipelines/global/scripts/average_bvecs.py”脚本替换为“HCPpipelines/global/scripts/average_bvecs.py”脚本
	${globalscriptsdir}/average_bvecs.py \
		${eddydir}/Pos.bval \
		${eddydir}/Pos_rotated.bvec \
		${eddydir}/Neg.bval \
		${eddydir}/Neg_rotated.bvec \
		${datadir}/avg_data \
		${CombineDataFlag} \
		${eddydir}/Pos_SeriesVolNum.txt \
		${eddydir}/Neg_SeriesVolNum.txt
...
if [ ! $GdCoeffs = "NONE" ] ; then
	...
	${globalscriptsdir}/GradientDistortionUnwarp.sh \
		--workingdir="${datadir}" \
		--coeffs="${GdCoeffs}" \
		--in="${datadir}/data_warped" \
		--out="${datadir}/data" \
		--owarp="${datadir}/fullWarp"
